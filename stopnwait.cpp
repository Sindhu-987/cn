...
NS_LOG_COMPONENT_DEFINE("StopAndWaitProtocol");
...
void SendPacket() {
    ...
    if (ackReceived) {
        ...
        // Retransmission timer (if ACK not received)
...
void ReceivePacket(Ptr<Socket> socket) {
    ...
    while ((packet = socket->Recv())) {
        ...
        if (type == 0) { // DATA packet
            ...
            NS_LOG_UNCOND("Sender: Got ACK at " << Simulator::Now().GetSeconds() << "s");
            ackReceived = true;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Mobility (for NetAnim visualization)
